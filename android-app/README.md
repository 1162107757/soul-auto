...
- **消息方向判断**：根据聊天气泡内头像的水平位置判断消息是"对方发的"还是"自己发的"（注：阈值按 1080px 宽屏幕计算，见「已知限制」）
...
1. **分辨率适配**：自动化层的部分坐标按 1080px 宽屏幕硬编码（如 540 的消息方向判定阈值、广场/列表的滑动坐标）。在 1080 宽、高 ≥1900px 的竖屏机型上表现最佳；720p、2K 宽屏、平板等机型可能出现消息方向误判或滑动失效，需要改代码适配。
